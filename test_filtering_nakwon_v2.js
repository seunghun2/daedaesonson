...
const EXCLUDE_KEYWORDS = [
    ...
    // 작업비 / 설치비 / 부대비용 (사용자 요청 2,3번 항목 제거)
    '작업비', '설치비', '개장', '수선', '이장', '파묘', '화장',
    '봉분', '리모델링', '토목', '공사', '각자', '글자', '철거', '운반',

    // 석물 부속 (묘지 자체가 아닌 부속품들)
    '상석', '비석', '와비', '둘레석', '묘테', '경계석', '석관', '석곽',
    '월석', '표석', '가족표석', '부부표석', '갓', '좌대', '판석', '석등', '걸방석',
...
// 3. 하지만 관리비여도 특정 조건은 제외할 수 있음 (예: '비석관리비' 같은게 있다면?) -> 일단 관리비는 다 살림.
...
try {
    ...
    rawItems.forEach(item => {
        ...
        if (!item.ExtractedName || item.ExtractedName.trim() === '') {
            rejectedItems.push({ name: '(Empty Name)', reason: 'Empty' });
            return;
        }
    ...
    // console.log('\n❌ [삭제된 항목]');
    // rejectedItems.forEach((r, i) => console.log(`${i+1}. ${r.name} (${r.reason})`));
