...
async function main() {
    const pdfPath = path.join(__dirname, '../archive/5.진주내동공원묘원/5.진주내동공원묘원_price_info.pdf');

    console.log('📄 PDF 분석 중: 5.진주내동공원묘원');
    ...
    console.log(`✅ 분석 완료: ${data.facilityName}`);
    console.log(`   총 ${data.items?.length || 0}개 항목`);

    // 2. 시트 데이터 형식으로 변환
    const facilityId = 'park-0005';
    const facilityName = data.facilityName || '진주내동공원묘원';
    ...
    sheetRows.slice(0, 5).forEach((row, i) => {
        console.log(`  ${i + 1}. ${row[5]} - ${row[7]}원 (${row[4]})`);
    });
    ...
    console.log(`\n✅ 완료! ${sheetRows.length}개 행이 69번 줄부터 삽입되었습니다.`);
}
