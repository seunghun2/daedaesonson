...
for (let i = 976; i <= 985; i++) {
    ...
    for (const g of sp) {
        for (const r of (g.rows || [])) {
            ...
            console.log(`  [${g.serviceType}] ${g.subType} → ${r.name} = ${(r.price || 0).toLocaleString()} | fee: ${r.feeType || '?'} | grade: ${r.grade || ''} | res: ${r.residency || ''} | gType: ${r.groupType || ''} ${rep}`);
        }
    }
}
