...
for (let i = 956; i <= 965; i++) {
    ...
    for (const g of sp) {
        for (const r of (g.rows || [])) {
            ...
            console.log(`  [${g.serviceType}] ${g.subType} → ${r.name} = ${(r.price || 0).toLocaleString()} | ${r.feeType || '?'} | grade: ${r.grade || ''} | res: ${r.residency || ''} | gType: ${r.groupType || ''} ${rep}`);
        }
    }
}
