...
// --- Configuration ---
const LIST_FILE_PATH = path.join(__dirname, '../esky_full_with_details.json');
...
(async () => {
    // Load List
    const rawData = JSON.parse(fs.readFileSync(LIST_FILE_PATH, 'utf-8'));
    const fullList = rawData.list || rawData;

    // Process ALL items
    console.log(`Loaded ${fullList.length} facilities.`);

    // Failure Log State
    const FAILURE_LOG_PATH = path.join(__dirname, '../crawling_failures.json');
    let failures = [];
    if (fs.existsSync(FAILURE_LOG_PATH)) {
        try {
            failures = JSON.parse(fs.readFileSync(FAILURE_LOG_PATH, 'utf-8'));
        } catch (e) { }
    }

    const saveFailure = (item, reason) => {
        failures.push({
            rno: item.rno,
    ...
    const browser = await puppeteer.launch({
        headless: false, // Keep it visible/headful as it's proven to work
    ...
    for (const item of fullList) {
        ...
        // Checkpoint: Skip if PDF already exists
        if (fs.existsSync(pdfPath)) {
            console.log(`[Skipping] #${rno} ${TARGET_FACILITY_NAME} - Already archived.`);
            continue;
        }

        console.log(`\n==================================================`);
        console.log(`Processing #${rno} (${++processedCount}/${fullList.length}): ${TARGET_FACILITY_NAME}`);
        ...
        try {
            ...
            // Search Strategy
            // Identify category hint from data if available, to prioritize tab?
            // item.facilitygroupcd might give a hint, but we'll stick to robust tab looping which works well.
            // TBC0700001 = Funeral Home? TBC0700002 = Cemetery? 
            // Let's rely on the Tab Loop.

            // Tabs to check.
            const tabsToCheck = ['묘지', '봉안시설', '자연장지', '장례식장'];
            // Optimization: If it is a cemetery, maybe start with '묘지'?
            // But let's just loop.
                    ...
                    if (!isNoResult) {
                        ...
                        const matchFound = await page.evaluate((targetName) => {
                            ...
                            for (let i = 0; i < items.length; i++) {
                                const li = items[i];
                                // Check if name matches. Be careful about spaces.
            ...
            // Sometimes it's '시설사진', sometimes '갤러리', sometimes '사진'
            const photoClicked = await clickTab(page, ['사진', '시설사진', '갤러리']);
            if (photoClicked) {
                await new Promise(r => setTimeout(r, 4000));

                const imageUrls = await page.evaluate(() => {
                    // Optimized Selector from inspection
                    ...
                    return uniqueImgs.filter(img => {
                        // Relaxed size filter, stricter anti-logo
            ...
            saveFailure(item, e.message);
        }

        // Random check: Wait a bit
