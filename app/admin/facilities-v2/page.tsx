        ...
        <Paper withBorder p="sm" radius="md" mb="sm">
            ...
            <Stack gap="xs">
                {groupData.rows.map((row: any, idx: number) => (
                    <Paper key={idx} withBorder p="xs" radius="sm" bg={row.isRepresentative ? 'yellow.0' : 'gray.0'}>
                        <Group align="flex-start">
                            <ActionIcon
                                variant={row.isRepresentative ? 'filled' : 'default'}
                                color={row.isRepresentative ? 'yellow' : 'gray'}
                                onClick={() => {
                                    let newRows = [...groupData.rows];
                                    const wasActive = newRows[idx].isRepresentative;

                                    if (wasActive) {
                                        // Toggle OFF
                                        newRows[idx] = { ...newRows[idx], isRepresentative: false };
                                    } else {
                                        // Turn ON (Radio: others OFF)
                            ...
                            <Stack gap={4} style={{ flex: 1 }}>
                                <Group grow>
                                    <TextInput
                                        ...
                                        variant="unstyled"
                                        fw={600}
                                        styles={{ input: { padding: 0, height: 'auto' } }}
                                    />
                                    <NumberInput
                                        placeholder="0"
                                        size="xs"
                                        ...
                                        variant="unstyled"
                                        styles={{ input: { padding: 0, height: 'auto', textAlign: 'right', fontWeight: 600, color: '#228be6' } }}
                                        min={0}
                                    />
                                </Group>
                                <TextInput
                                    placeholder="설명 (예: 1평/영구)"
                                    size="xs"
                                    value={row.description || row.grade || ''}
                                    onChange={(e) => {
                                        const newRows = [...groupData.rows];
                                    ...
                                    variant="unstyled"
                                    c="dimmed"
                                    styles={{ input: { padding: 0, height: 'auto', fontSize: '11px' } }}
                                />
                            </Stack>
...
export default function AdminPage() {
        ...
        if (parsedFacility.id) {
            try {
                ...
                if (response.ok) {
                    ...
                    // 🌟 Gold Standard Data Mapping (pricing -> priceInfo.priceTable)
                    let finalPriceTable = detailedData.priceTable;

                    // If 'pricing' exists (New Standard), convert it to 'priceInfo.priceTable' (UI Standard)
                    if (detailedData.pricing && Object.keys(detailedData.pricing).length > 0) {
                        console.log('💎 Found Gold Standard Pricing, mapping to UI...');
                        finalPriceTable = {};
                        Object.entries(detailedData.pricing).forEach(([category, data]: [string, any]) => {
                            // data.rows가 배열이라고 가정
                            if (data && Array.isArray(data.rows)) {
                                finalPriceTable[category] = {
                                    rows: data.rows,
                                    unit: '원', // Default unit
                                    category: 'basic' // Default logic will handle tab assignment
                                };
                            }
                        });
                    }
                    ...
                    console.log('✅ Loaded detailed prices from DB (Mapped):', detailedData._meta);
                } else {
                    // Fallback to JSON data

                    // Also check for local 'pricing' field in parsedFacility (from facilities.json)
                    if (parsedFacility.pricing) {
                        console.log('💎 Found Local Gold Standard Pricing in JSON, mapping...');
                        const localPriceTable: any = {};
                        Object.entries(parsedFacility.pricing).forEach(([category, data]: [string, any]) => {
                            if (data && Array.isArray(data.rows)) {
                                localPriceTable[category] = {
                                    rows: data.rows,
                                    unit: '원'
                                };
                            }
                        });
                        parsedFacility.priceInfo = { priceTable: localPriceTable };
                    }
    ...
    const handleSave = async () => {
        try {
            // alert('저장 로직 시작!'); // User requested aggressive debug, but let's be slightly gentle. 
            // Actually, user said "1 안떠!". Let's show start message to confirm button click works.
            ...
            // alert(`Checking PriceTable: ${editForm.priceInfo?.priceTable ? 'Exists' : 'Missing'}`);
            ...
            if (editForm.priceInfo?.priceTable) {
                ...
                if (editForm.category === 'FAMILY_GRAVE') preferredKeywords = ['매장', '묘지'];
                else if (editForm.category === 'CHARNEL_HOUSE') preferredKeywords = ['봉안', '납골'];
                else if (editForm.category === 'NATURAL_BURIAL') preferredKeywords = ['수목', '자연', '잔디', '화초'];

                Object.entries(editForm.priceInfo.priceTable).forEach(([groupName, groupData]: [string, any]) => {
                    // Skip non-product costs
                    if (groupName.includes('관리비') || groupName.includes('유지') || groupName.includes('추가') ||
                        groupName.includes('별도') || groupName.includes('안내') || groupName.includes('설치') ||
                        groupName.includes('조경') || groupName.includes('용품')) {
                        return;
                    }

                    if (groupData && Array.isArray(groupData.rows)) {
                        groupData.rows.forEach((row: any) => {
                            ...
                            if (rowName.includes('관리비') || rowName.includes('연회비') || rowName.includes('부대비용') ||
                                ...
                                rowName.includes('표석') || rowName.includes('식재') || rowName.includes('제거') ||
                            ...
                            if (!isNaN(price) && price > 0) {
                                ...
                                if (price > absoluteMax) absoluteMax = price;
                                hasPrice = true;

                                // Check Representative Price Logic
                                if (row.isRepresentative) {
                                    if (preferredKeywords.length > 0) {
                                        if (preferredKeywords.some(k => groupName.includes(k) || rowName.includes(k))) {
                                            if (representativePrice === null) representativePrice = price;
                                        }
                                    } else {
                                        if (representativePrice === null) representativePrice = price;
                                    }
                ...
                // --- DEBUG START ---
                let debugMsg = `[DEBUG] 카테고리: ${editForm.category}\n키워드: ${preferredKeywords.join(', ')}\n`;
                debugMsg += `찾은 대표가격(Raw): ${representativePrice}\n`;
                debugMsg += `최저가(Min): ${absoluteMin}\n`;
                debugMsg += `별표 체크 여부: ${hasPrice ? '가격있음' : '가격없음'}`;
                alert(debugMsg);
                // --- DEBUG END ---

                if (hasPrice) {
                    const targetMin = representativePrice !== null ? representativePrice : absoluteMin;
                    const finalMin = targetMin >= 10000 ? Math.round(targetMin / 10000) : targetMin;
                    const finalMax = absoluteMax >= 10000 ? Math.round(absoluteMax / 10000) : absoluteMax;
            ...
            console.error('handleSave Critical Error:', error);
            alert('저장 중 심각한 오류 발생: ' + String(error));
        }
    };
